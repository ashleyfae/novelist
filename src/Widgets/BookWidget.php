...
class BookWidget extends WP_Widget
{
    ...
    public function form($instance)
    {
        ...
        <p>
            ...
            <?php if ($book_query->have_posts()) : ?>
                <select class="widefat" id="<?php echo $this->get_field_id('book'); ?>" name="<?php echo $this->get_field_name('book'); ?>">
                    <option value="" <?php selected($book_id, ''); ?>>
                        <?php esc_html_e('- Select a Book -', 'novelist'); ?>
                    </option>
                    <?php while ($book_query->have_posts()) : $book_query->the_post(); ?>
                        <option
                            value="<?php echo esc_attr(get_the_ID()); ?>"
                            <?php selected($book_id, get_the_ID()); ?>
                        >
            ...
            <?php else : ?>
                ...
                <?php
                printf(
                    __('It looks like you don\'t have any %1$s yet! Why not <a href="%2$s">add one?</a>', 'novelist'),
                    novelist_get_label_plural(),
                    esc_url(admin_url('/post-new.php?post_type=book'))
                );
                ?>
            <?php endif; ?>
        ...
        <p><strong><?php esc_html_e('Book Cover Settings', 'novelist'); ?></strong></p>

        <p class="novelist-row">
            <input
                id="<?php echo $this->get_field_id('show_book_cover'); ?>"
                class="novelist-checkbox-change"
                data-target=".novelist-show-book-cover-contents"
                name="<?php echo $this->get_field_name('show_book_cover'); ?>"
                type="checkbox" value="on"
                <?php checked($show_book_cover, 'on'); ?>
            >
            <label for="<?php echo $this->get_field_id('show_book_cover'); ?>">
                <?php esc_html_e('Show book cover', 'novelist'); ?>
            </label>
        </p>
        ...
        <div class="novelist-show-book-cover-contents">
            <table>
                <tr class="novelist-row">
                    <td>
                        <label for="<?php echo $this->get_field_id('book_cover_size'); ?>">
                            <?php esc_html_e('Size:', 'novelist'); ?>
                        </label>
                        <select
                            class="widefat novelist-checkbox-change"
                            data-target=".novelist-custom-size-content"
                            data-target-value="custom"
                            id="<?php echo $this->get_field_id('book_cover_size'); ?>"
                            name="<?php echo $this->get_field_name('book_cover_size'); ?>"
                        >
                            <?php foreach (novelist_get_image_sizes() as $key => $value) : ?>
                                <option
                                    value="<?php echo esc_attr($key); ?>"
                                    <?php selected($book_cover_size, esc_attr($key)); ?>
                                >
                    ...
                    <td>
                        <label for="<?php echo $this->get_field_id('book_cover_align'); ?>">
                            <?php esc_html_e('Alignment:', 'novelist'); ?>
                        </label>
                        <select
                            class="widefat"
                            id="<?php echo $this->get_field_id('book_cover_align'); ?>"
                            name="<?php echo $this->get_field_name('book_cover_align'); ?>"
                        >
                            <?php foreach ($this->get_alignments() as $key => $value) : ?>
                                <option
                                    value="<?php echo esc_attr($key); ?>"
                                    <?php selected($book_cover_align, esc_attr($key)); ?>
                                >
                ...
                <tr class="novelist-custom-size-content">
                    <td>
                        <label for="<?php echo $this->get_field_id('book_cover_width'); ?>">
                            <?php esc_html_e('Width:', 'novelist'); ?>
                        </label>
                        <input
                            class="widefat"
                            id="<?php echo $this->get_field_id('book_cover_width'); ?>"
                            name="<?php echo $this->get_field_name('book_cover_width'); ?>"
                            type="number"
                            value="<?php echo esc_attr($book_cover_width); ?>"
                        >
                    </td>
                    <td>
                        <label for="<?php echo $this->get_field_id('book_cover_height'); ?>">
                            <?php esc_html_e('Height:', 'novelist'); ?>
                        </label>
                        <input
                            class="widefat"
                            id="<?php echo $this->get_field_id('book_cover_height'); ?>"
                            name="<?php echo $this->get_field_name('book_cover_height'); ?>"
                            type="number"
                            value="<?php echo esc_attr($book_cover_height); ?>"
                        >
                    </td>
                </tr>
            </table>
        ...
        <p>
            <input
                id="<?php echo $this->get_field_id('show_synopsis'); ?>"
                name="<?php echo $this->get_field_name('show_synopsis'); ?>"
                type="checkbox"
                value="on"
                <?php checked($show_synopsis, 'on'); ?>
            >
            <label for="<?php echo $this->get_field_id('show_synopsis'); ?>">
                <?php esc_html_e('Show synopsis', 'novelist'); ?>
            </label>
        </p>
        ...
        <p class="novelist-row">
            <input
                ...
                class="novelist-checkbox-change"
                data-target=".novelist-show-link-contents"
                name="<?php echo $this->get_field_name('show_link'); ?>"
                type="checkbox"
                value="on"
                <?php checked($show_link, 'on'); ?>
            >
            <label for="<?php echo $this->get_field_id('show_link'); ?>">
                <?php esc_html_e('Show link to book page', 'novelist'); ?>
            </label>
        </p>
        ...
        <div class="novelist-show-link-contents">
            <p>
                <label for="<?php echo $this->get_field_id('link_text'); ?>">
                    <?php esc_html_e('Link Text:', 'novelist'); ?>
                </label>
                <input
                    class="widefat"
        ...
        <p><strong><?php esc_html_e('Extra Text', 'novelist'); ?></strong></p>

        <p>
            <label for="<?php echo $this->get_field_id('text_before'); ?>">
                <?php esc_html_e('Text before the book:', 'novelist'); ?>
            </label>
            <textarea
                class="widefat"
                id="<?php echo $this->get_field_id('text_before'); ?>"
                name="<?php echo $this->get_field_name('text_before'); ?>"
                rows="10"
            ><?php echo esc_textarea($text_before); ?></textarea>
        </p>

        <p>
            <label for="<?php echo $this->get_field_id('text_after'); ?>">
                <?php esc_html_e('Text after the book:', 'novelist'); ?>
            </label>
            <textarea
                class="widefat"
                id="<?php echo $this->get_field_id('text_after'); ?>"
                name="<?php echo $this->get_field_name('text_after'); ?>"
                rows="10"
            ><?php echo esc_textarea($text_after); ?></textarea>
        </p>
        <?php
    ...
    public function update($new_instance, $old_instance): array
    {
        ...
        $instance['book_cover_size']   = (! empty($new_instance['book_cover_size']) && array_key_exists($new_instance['book_cover_size'],
                novelist_get_image_sizes())) ? strip_tags($new_instance['book_cover_size']) : 'medium';
        $instance['book_cover_align']  = (! empty($new_instance['book_cover_align']) && array_key_exists($new_instance['book_cover_align'],
                $this->get_alignments())) ? strip_tags($new_instance['book_cover_align']) : 'center';
        ...
        $instance['link_text']         = (! empty($new_instance['link_text'])) ? wp_kses($new_instance['link_text'],
    ...
    public function get_image_sizes(): array
    {
        ...
        return (array) apply_filters('novelist/widget/book/image-sizes', $sizes);
    }
     ...
     * @return array
     */
    public function get_alignments(): array
    {
        $alignments = array(
        ...
        return (array) apply_filters('novelist/widget/book/image-alignments', $alignments);
    }
    ...
    public function get_allowed_html(): array
    {
        ...
        return (array) apply_filters('novelist/widget/book/allowed-link-text-html', $allowed_html);
    }
