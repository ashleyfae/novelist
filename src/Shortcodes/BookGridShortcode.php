...
class BookGridShortcode implements Shortcode
{
    ...
    public function make($atts, $content = ''): string
    {
        ...
        if (
            ...
            (! filter_var($atts['pagination'], FILTER_VALIDATE_BOOLEAN) && $atts['number'] > 0)
        ) {
        ...
        if ($books->have_posts()) {
            ...
            ob_start(); ?>
            <div class="novelist-book-list <?php echo apply_filters('novelist/shortcode/book/wrapper-class', $class,
                $atts); ?>">
                        ...
                        if ($atts['display']) {
                            ...
                            if (count($extra_content)) {
                                foreach ($extra_content as $content) {
                                    do_action(
                                        'novelist/shortcode/book/layout-display',
                                        trim($content),
                                        get_the_ID(),
                                        $atts
                                    );
